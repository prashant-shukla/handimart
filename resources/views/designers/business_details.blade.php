...
@endif
<!-----  If Client is designers         ---->
<div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
  <div>
    <h4 class="mb-3 mb-md-0">Become A Designer</h4>
  </div>
...
<div class="col-md-12 col-xl-12 middle-wrapper">
        ...
        <form action="{{url('dashboard/designers/business-profile/business_submit/'.$id)}}" method="post" class="parsley-examples" id="business_details_form"  enctype="multipart/form-data">
          @csrf
          <div class="row">
            <div class="col-sm-6">
            ...
            <div class="col-sm-6">
              <div class="form-group">
                ...
                <select name='category' class="form-control">     
                  ...
                  @if(count($designer_categories) > 0)
                    @foreach ($designer_categories as $designer_cat)
                      <option value="{{$designer_cat->id}}" >{{$designer_cat->name}}</option>
                    @endforeach
                  @endif
                </select>
              </div>
            ...
            <div class="col-sm-6">
              <div class="form-group mb-0">
                ...
                <div class="row">
                    ...
                    @if(count($designer_skills) > 0) 
                    @php $indexVal = 1; @endphp
                    @foreach ($designer_skills as $designer_skill)
                    <div class="col-sm-4">
                    <div class="custom-control custom-checkbox mb-2">
                      <input name='skills[]' value='{{$designer_skill->name}}' type="checkbox" class="custom-control-input" id="customCheck{{$indexVal}}">
                      <label class="custom-control-label" for="customCheck{{$indexVal}}">{{$designer_skill->name}}</label>
                    </div>
                  </div>
                      @php $indexVal++; @endphp
                    @endforeach
            ...
            <div class="col-sm-6">
              <div class="form-group">
                ...
                <select name='category' class="form-control">     
                  ...
                  @if(count($designer_categories) > 0)
                    @foreach ($designer_categories as $designer_cat)
                      <option value="{{$designer_cat->id}}" @if($designer_cat->id==$business_detail->category ) selected @endif >{{$designer_cat->name}}</option>
                    @endforeach
                  @endif
                </select>
              </div>
            ...
            <div class="col-sm-6">
              <div class="form-group mb-0">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4">
                    <div class="custom-control custom-checkbox mb-2">
                      <input name='skills[]' value='{{$designer_skill->name}}' type="checkbox" class="custom-control-input" id="customCheck{{$indexVal}}" @if (strpos($business_detail->skills, $designer_skill->name) !== false) checked @endif>
                      <label class="custom-control-label" for="customCheck{{$indexVal}}">{{$designer_skill->name}}</label>
                    </div>
                  </div>
                      @php $indexVal++; @endphp
