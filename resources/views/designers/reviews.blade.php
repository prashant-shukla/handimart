...
<nav class="page-breadcrumb">
  <ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="{{ url('dashboard/designers/jobs/'.$id) }}">Jobs</a></li>
              ...
              <div class="tab-content p-top-3" id="myTabContent">
                ...
                <div class="tab-pane fade active show" id="received" role="tabpanel" aria-labelledby="received-tab">
                  <div class="row" >
                    ...
                    $count++;
                    @endphp
                      <div class="col-md-6" id="reviewOuterDiv_{{$recevied->id}}">
                        <div class="card rounded">
                          <div class="card-header">
                            <div class="d-flex align-items-center justify-content-between">
                              <div class="d-flex align-items-center">
                                <img class="img-xs rounded-circle" src="{{ $recevied->sender_data->image_thumb_path }}" alt="">
                                <div class="ml-2">
                                  <p><a href="{{ url('dashboard/designers/my-profile/profile/'.$id.'/'.$recevied->sender_id) }}" class=" userNameReview" style="display: inline-block;">{{$recevied->sender_name}},
                                    <span>({{ $recevied->sender_data->user_id }})</span></a></p>
                                  <p class="tx-11 text-muted">{{ ucfirst($recevied->sender_data->public_name) }}</p>
                                </div>
                              </div>
                              <div class="dropdown">
                                ...
                                @endfor
                                @if($recevied->ratings < 5)
                                @php
                                $less_rating = 5 - $recevied->ratings;
                                @endphp
                          ...
                          <div class="card-body">
                            ...
                            <p class="mb-3 tx-14">{{$recevied->description}}</p>
                            
                            @if(isset($recevied->review_images))
                              <div class="row">
                              @foreach($recevied->review_images as $review_image)
                                <div class="col-md-3">
                            ...
                            <label class="review-date">
                              <i class="dateicon" data-feather="calendar"></i> {{ date('F jS, Y', strtotime($recevied->created_at)) }}
                            </label>

                            <!-- Reply Section -->
                            @if(isset($recevied->review_reply)  && count($recevied->review_reply) > 0)
                            <h6 class="card-sub-title mb-2 mt-2">
                                Replies
                            </h6>
                            <a data-toggle="collapse" href="#collapseExample{{$recevied->id}}"
                              role="button" aria-expanded="false" aria-controls="collapseExample">
                              See All Replies
                            </a>
                            <div class="collapse mt-1" id="collapseExample{{$recevied->id}}">
                              @foreach($recevied->review_reply as $review_reply)
                              <div class="d-flex align-items-center justify-content-between">
                                <div class="d-flex align-items-center">
                                  <img class="img-xs rounded-circle" src="{{ $review_reply->user_image_thumb_path }}" alt="">
                                  <div class="ml-2">
                                    <p><a href="{{ url('dashboard/designers/my-profile/profile/'.$id.'/'.$review_reply->sender_id) }}" class=" userNameReview" style="display: inline-block;">{{$review_reply->name}},
                          ...
                          <div class="card-footer">
                            <div class="d-flex post-actions">
                              <a href="javascript:;" class="d-flex align-items-center text-muted mr-4 ReplyReview" data-review_id="{{$recevied->id}}" data-id="{{$id}}">
                                <i class="icon-md" data-feather="corner-up-left"></i>
                                <p class="d-none d-md-block ml-2 mt-1">Reply</p>
                              </a>
                          ...
                          <div class="card-header">
                            <div class="d-flex align-items-center justify-content-between">
                              <div class="d-flex align-items-center">
                                ...
                                <div class="ml-2">
                                  <p><a href="{{ url('dashboard/designers/my-profile/profile/'.$id.'/'.$submitted->receiver_id) }}" class=" userNameReview" style="display: inline-block;">{{$submitted->receiver_name}},
...
<script>
  var root_path = "{{URL::to('dashboard/designers/reviews')}}";
