...
<div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
  <div>
    <h4 class="mb-3 mb-md-0">Become A Painter</h4>
  </div>
  ...
  <div class="row">
    <div class="col-md-12">
       <div class="card rounded py-5">
        ...
        <form action="{{url('dashboard/painters/business-profile/business_submit/'.$id)}}" method="post" class="parsley-examples" id="business_details_form"  enctype="multipart/form-data">
          @csrf
          <div class="row">
            <div class="col-sm-6">
            ...
            <div class="col-sm-6">
              <div class="form-group">
                ...
                <select name='category' class="form-control">     
                  ...
                  @if(count($painter_categories) > 0)
                    @foreach ($painter_categories as $painter_cat)
                      <option value="{{$painter_cat->id}}" >{{$painter_cat->name}}</option>
                    @endforeach
                  @endif
                </select>
              </div>
          ...
          <div class="row">
            <div class="col-sm-6">
              <div class="form-group mb-0">
                ...
                <div class="row">
                    @if(count($painter_skills) > 0) 
                    @php $indexVal = 1; @endphp
                    @foreach ($painter_skills as $painter_skill)
                    <div class="col-sm-4">
                    <div class="custom-control custom-checkbox mb-2">
                      <input name='skills[]' value='{{$painter_skill->name}}' type="checkbox" class="custom-control-input" id="customCheck{{$indexVal}}">
                      <label class="custom-control-label" for="customCheck{{$indexVal}}">{{$painter_skill->name}}</label>
                    </div>
                  </div>
                    @php $indexVal++; @endphp
                    @endforeach
            ...
            <div class="col-sm-6">
              <div class="form-group">
                ...
                <select name='category' class="form-control">     
                  ...
                  @if(count($painter_categories) > 0)
                    @foreach ($painter_categories as $painter_cat)
                      <option value="{{$painter_cat->id}}" @if($painter_cat->id==$business_detail->category ) selected @endif >{{$painter_cat->name}}</option>
                    @endforeach
                  @endif
                </select>
              </div>
            ...
            <div class="col-sm-6">
              <div class="form-group mb-0">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4">
                    <div class="custom-control custom-checkbox mb-2">
                      <input name='skills[]' value='{{$painter_skill->name}}' type="checkbox" class="custom-control-input" id="customCheck{{$indexVal}}" @if (strpos($business_detail->skills, $painter_skill->name) !== false) checked @endif>
                      <label class="custom-control-label" for="customCheck{{$indexVal}}">{{$painter_skill->name}}</label>
                    </div>
                  </div>
                      @php $indexVal++; @endphp
