...
@endif
<!-----  If Client is photographers         ---->
<div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
  <div>
    <h4 class="mb-3 mb-md-0">Become A Photographers</h4>
  </div>
...
<div class="col-md-12 col-xl-12 middle-wrapper">
  <div class="row">
      <div class="col-md-12">
        ...
        <form action="{{url('dashboard/photographers/business-profile/business_submit/'.$id)}}" method="post" class="parsley-examples" id="business_details_form"  enctype="multipart/form-data">
          @csrf
          <div class="row">
            <div class="col-sm-6">
            ...
            <div class="col-sm-6">
              <div class="form-group">
                ...
                <select name='category' class="form-control">     
                  ...
                  @if(count($photographer_categories) > 0)
                    @foreach ($photographer_categories as $photographer_cat)
                      <option value="{{$photographer_cat->id}}" >{{$photographer_cat->name}}</option>
                    @endforeach
                  @endif
                </select>
              </div>
            ...
            <div class="col-sm-6">
              <div class="form-group mb-0">
                ...
                <div class="row">
                    ...
                    @if(count($photographer_skills) > 0) 
                    @php $indexVal = 1; @endphp
                    @foreach ($photographer_skills as $photographer_skill)
                    <div class="col-sm-4">
                    <div class="custom-control custom-checkbox mb-2">
                      <input name='skills[]' value='{{$photographer_skill->name}}' type="checkbox" class="custom-control-input" id="customCheck{{$indexVal}}">
                      <label class="custom-control-label" for="customCheck{{$indexVal}}">{{$photographer_skill->name}}</label>
                    </div>
                  </div>
                      @php $indexVal++; @endphp
                    @endforeach
        ...
        <form action="{{url('dashboard/photographers/business-profile/business_submit/'.$id)}}" method="post" class="parsley-examples" id="business_details_form" enctype="multipart/form-data">
            ...
            <div class="col-sm-6">
              <div class="form-group">
                ...
                <select name='category' class="form-control">     
                  ...
                  @if(count($photographer_categories) > 0)
                    @foreach ($photographer_categories as $photographer_cat)
                      <option value="{{$photographer_cat->id}}" @if($photographer_cat->id==$business_detail->category ) selected @endif >{{$photographer_cat->name}}</option>
                    @endforeach
                  @endif
                </select>
              </div>
          ...
          <div class="row">
            ...
            <div class="col-sm-6">
              <div class="form-group">
                <label class="control-label">Per Day Fee</label>
                <input name="per_day_fee"  type="text" class="form-control" placeholder="Enter Per Day Fee" value="{{ $business_detail->per_day_fee }}">
              </div>
            </div><!-- Col -->
          </div><!-- Row -->
          <div class="row">
            ...
            <div class="col-sm-6">
              <div class="form-group mb-0">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4">
                    <div class="custom-control custom-checkbox mb-2">
                      <input name='skills[]' value='{{$photographer_skill->name}}' type="checkbox" class="custom-control-input" id="customCheck{{$indexVal}}" @if (strpos($business_detail->skills, $photographer_skill->name) !== false) checked @endif>
                      <label class="custom-control-label" for="customCheck{{$indexVal}}">{{$photographer_skill->name}}</label>
                    </div>
                  </div>
                      @php $indexVal++; @endphp
